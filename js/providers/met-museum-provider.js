...
class MetMuseumProvider extends ArtProvider {
  ...
  async getAsset(index) {
    ...
    if (this.syncedAssetData[index]) {
      return this.syncedAssetData[index];
    }
    
    // Try to fetch a valid asset starting from the requested index
    for (let i = index; i < Math.min(index + 5, this.allAvailableIds.length); i++) {
      if (this.syncedAssetData[i]) {
        return this.syncedAssetData[i];
      }
  ...
  async getDisplayImageUrl(assetId) {
    if (assetId < 0 || !this.allAvailableIds || assetId >= this.allAvailableIds.length) {
      return null;
    }
